...
async function getResForChannel(channel) {
    ...
    const isDotaChannel = (video) => video.channel.name.toLowerCase() !== 'jenkins' || ( video.channel.name.toLowerCase() === 'jenkins' && video.channel.id === "UCTUO_3bYr50vmH0aE7byMjg" );
    results = res.videos.filter((v) => v.channel.name.toLowerCase() === channel.toLowerCase() && isDotaChannel(v));
    return results;
}
